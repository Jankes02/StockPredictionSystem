...
Trains an LSTM next-day-direction classifier on the anchored training window
and evaluates it out-of-sample on exactly the same trading calendar, costs, and
position sizing as the ensemble headline run. The LSTM's long/flat signals are
executed through the shared `PortfolioSimulator`, so the comparison isolates
the decision rule rather than the accounting.

Requires the headline walk-forward run for the same config first (it supplies
the OOS window, the ensemble equity curve, and the train/test split):
...
def _backtest_lstm(
    ...
    for date in oos_dates:
        ...
        for sym in symbols:
            ...
            if pd.isna(prob):
                continue
            action = "BUY" if float(prob) >= LONG_THRESHOLD else "SELL"
            decisions.append(
                {
                    "symbol": sym,
                    "action": action,
...
def main() -> None:
    ...
    model.fit(data_by_symbol, train_end)

    print(f"Backtesting LSTM over {len(oos_dates)} OOS days ...")
    panel = model.predict_prob_panel(data_by_symbol)
    lstm_portfolio = _backtest_lstm(panel, data_by_symbol, oos_dates, bt)
    ...
    pd.DataFrame(lstm_curve).to_csv(
        RESULTS_DIR / "ml_baseline_equity.csv", index=False
    )
