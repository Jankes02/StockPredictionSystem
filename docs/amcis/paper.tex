...
\begin{abstract}
We present the design and evaluation of a multi-agent decision support system for stock trading on the Warsaw Stock Exchange, focused on WIG20 index constituents. Several technical-analysis agents (MACD, RSI, ROC, Bollinger Bands, and moving-average trend) produce signals with confidence values; one decision agent aggregates them and produces buy, sell, or hold recommendations with configurable behavior and a minimum confidence threshold. We backtest the system on daily data from May 2021 to January 2026 against buy-and-hold WIG20. The best configuration achieves 88\% total return versus 53\% for the index, with a steadier equity path, Sharpe ratio 1.3, and maximum drawdown 12\%. The results support the view that this modular multi-agent design can serve as a usable decision support tool and contribute to IS research on decision support in financial markets.

\emph{\textbf{Keywords:} decision support systems, algorithmic trading, multi-agent systems, technical analysis, stock trading, Warsaw Stock Exchange, WIG20, backtesting, financial markets}
\end{abstract}
...
The equity of system strategy is much more stable in comparison to the buy-and-hold strategy. The former represents a steady increase of equity, heavily outperforming the latter over a long period.
