...
%DIF ADD paper.tex       Mon Jul 13 11:53:25 2026
...
\cite{li2025consensus}}\hskip0pt%DIFAUXCMD
...
Although the Warsaw Stock Exchange is the largest market in Central and Eastern Europe, it is underrepresented in the algorithmic trading literature. Studies of the WSE have focused on \DIFdelbegin \DIFdel{correlation structure and dependence among CEE markets \mbox{%DIFAUXCMD
\cite{czapkiewicz2015}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{the interdependence between the Polish and other stock markets and its macroeconomic drivers \mbox{%DIFAUXCMD
\cite{czapkiewicz2018}}\hskip0pt%DIFAUXCMD
}\DIFaddend , on tests of the \DIFdelbegin \DIFdel{efficient markets hypothesis using agent-based simulation \mbox{%DIFAUXCMD
\cite{jagric2015}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{informational efficiency of the exchange \mbox{%DIFAUXCMD
\cite{kompa2009}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and on the \DIFdelbegin \DIFdel{emergence of algorithmic tradingon WIG20 constituents \mbox{%DIFAUXCMD
\cite{buczynski2021}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{interplay between algorithmic trading, liquidity, and volatility for WSE blue-chip stocks \mbox{%DIFAUXCMD
\cite{gurgul2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend . To the best of our knowledge, multi-agent decision support systems have not been evaluated on WIG20 under an explicit walk-forward protocol with transaction costs and a risk-adjusted comparison. This gap is the \DIFaddbegin \DIFadd{primary }\DIFaddend empirical niche of the present paper\DIFaddbegin \DIFadd{; to test whether the resulting profile is specific to WSE conditions, we additionally replicate the full pipeline on the FTSE 100 under UK-specific transaction costs (Section~\ref{subsec:ftse})}\DIFaddend .
...
\DIFaddbegin \DIFadd{H1 is stated against buy-and-hold WIG20, because that is the primary panel; Section~\ref{subsec:ftse} replicates the same risk-adjusted comparisons on the FTSE 100 as supplementary external evidence. H2 and H3 are evaluated on WIG20 only.
}

\DIFaddend Section~\ref{sec:results} reports the evidence for each hypothesis, and Section~\ref{sec:discussion} discusses the degree to which the evidence is consistent with the theoretical predictions.
...
\DIFdelbegin \DIFdel{The dataset consists of daily closing prices of }\DIFdelend \DIFaddbegin \DIFadd{Both experiments use daily OHLCV panels from a public repository, Stooq.}\footnote{\url{https://stooq.com}} \DIFadd{On each market, the ensemble, the buy-and-hold index benchmark, and the LSTM directional baseline trade the same fixed roster of constituents; incomplete rows are dropped per symbol. The primary panel covers }\DIFaddend the WIG20 \DIFdelbegin \DIFdel{constituent companies}\DIFdelend \DIFaddbegin \DIFadd{index and nineteen WIG20 stocks on the Warsaw Stock Exchange}\DIFaddend , excluding \.{Z}abka Group SA.\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{\, }\DIFaddend whose listing on $2024/10/17$ is much more recent than that of the remaining constituents. The period covers \DIFdelbegin \DIFdel{$05/26/2021$ to $03/31/2026$, which yields }\DIFdelend \DIFaddbegin \DIFadd{$2021/05/26$ to $2026/03/31$ (}\DIFaddend roughly $1{,}220$ trading days\DIFdelbegin \DIFdel{. For every symbol, incomplete rows are dropped. All prices }\DIFdelend \DIFaddbegin \DIFadd{). Values }\DIFaddend are denominated in \DIFdelbegin \DIFdel{Polish z}%DIFDELCMD < {%%%
\DIFdel{\l}%DIFDELCMD < }%%%
\DIFdel{oty (PLN). The data were retrieved from the public repository, Stooq. }\footnote{%DIFDELCMD < \url{https://stooq.com}%%%
} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{The same data source and symbol set are used for both the strategy and the buy-and-hold WIG20 baseline, so that the two seriesshare a common set of dates and currency basis}\DIFdelend \DIFaddbegin \DIFadd{PLN. External validation uses }\FtseNsymbols{} \DIFadd{liquid FTSE~100 constituents and the FTSE~100 index on the London Stock Exchange over the same calendar window in GBP. The anchored split is identical on both markets (train through $2024/06/30$, test from $2024/07/01$ through $2026/03/31$); agents, hyperparameters, and evaluation procedures are unchanged, and only the price panels, index series, and transaction-cost calibration differ (Sections~\ref{subsec:costs} and~\ref{subsec:ftse})}\DIFaddend .
...
\DIFdelbegin \DIFdel{Two }\DIFdelend \DIFaddbegin \DIFadd{Three }\DIFaddend families of baselines are used. The first is \emph{\DIFaddbegin \DIFadd{index }\DIFaddend buy-and-hold\DIFdelbegin \DIFdel{WIG20}\DIFdelend }\DIFdelbegin \DIFdel{, an equity curve that }\DIFdelend \DIFaddbegin \DIFadd{: on WIG20 the benchmark }\DIFaddend invests the initial cash into the WIG20 index on the first date of the test window and holds the position until the end\DIFaddbegin \DIFadd{; on the FTSE replication the same rule applies to the FTSE 100 index}\DIFaddend . Buy-and-hold is a standard benchmark in the evaluation of trading strategies \cite{dichtl2020}. The second family is a set of \emph{single-agent} runs in which the decision agent receives signals from only one signal agent; this produces five baselines, one per indicator. In addition, we run five \emph{leave-one-agent-out} configurations, each of which removes one agent from the ensemble. \DIFaddbegin \DIFadd{The third family is a }\emph{\DIFadd{deep-learning baseline}}\DIFadd{, described next. }\DIFaddend All baselines are evaluated on the same test window \DIFdelbegin \DIFdel{, use the same transaction costs }\DIFdelend \DIFaddbegin \DIFadd{on each market, use market-specific transaction costs described in Sections~\ref{subsec:costs} and~\ref{subsec:ftse}}\DIFaddend , and adopt the same decision-agent configuration selected by the walk-forward protocol. The only \DIFdelbegin \DIFdel{exception is }\DIFdelend \DIFaddbegin \DIFadd{exceptions are }\DIFaddend buy-and-hold, which has no decision-agent parameters\DIFaddbegin \DIFadd{, and the deep-learning baseline, which replaces the decision agent with a learned classifier.
}
...
\begin{table}[t]
\centering
\caption{In-sample total return (train window, $2021$\DIFdelbeginFL \DIFdelFL{--}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-}\DIFaddendFL $2024$) by decision-agent mode and minimum confidence. Higher is better. Selected configuration in bold.}
...
\begin{figure}[t]
    ...
    \caption{Heatmap of the pairwise Pearson correlations of the agent signed-confidence signals on the training window. Blue cells mark negative correlations, red cells mark positive ones. The largest absolute correlations appear in the \DIFdelbeginFL \DIFdelFL{RSI--ROC }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{RSI-ROC }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{RSI--Bollinger }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{RSI-Bollinger }\DIFaddendFL pairs, which is consistent with their shared reliance on overbought/oversold-style rules.}
...
Equation~\eqref{eq:combined_variance} predicts that the combined forecast error is strictly smaller than the smallest individual forecast error whenever the individual errors are not perfectly correlated. Table~\ref{tab:corr} and Fig.~\ref{fig:corr_matrix} show that the empirical pairwise correlations of the agent signals span the full range from $\CorrMin{}$ to $\CorrMax{}$, with a mean absolute off-diagonal correlation of $\CorrMeanAbs{}$. The theoretically most informative pairs are those with negative correlations, in particular RSI vs.\ ROC ($\CorrMin{}$) and Bollinger vs.\ ROC ($-0.55$). The \DIFdelbegin \DIFdel{RSI--ROC }\DIFdelend \DIFaddbegin \DIFadd{RSI-ROC }\DIFaddend pair is especially diagnostic: RSI is a mean-reversion rule that emits a buy signal in oversold conditions, whereas ROC is a momentum rule that emits a sell signal in the same conditions. The two agents therefore naturally disagree, and the score of the decision agent resolves the disagreement by weighing confidences across families. In a window dominated by the bull regime, the trend family (MA trend) dominates the score; in sideways or correction regimes, the mean-reversion and volatility families gain relative weight. The regime decomposition in Table~\ref{tab:regime} shows the footprint of this mechanism: the ensemble loses less than buy-and-hold during corrections and earns more during sideways regimes, which is precisely the pattern predicted by a variance-minimizing combination of agents with heterogeneous error structures.
...
The headline results, read together with the risk-adjusted table, the volatility-matched table, and the regime decomposition, \DIFdelbegin \DIFdel{provide strong support for }\DIFdelend \DIFaddbegin \DIFadd{are consistent with }\DIFaddend H1. On the aggregate test window, the ensemble trails buy-and-hold on gross total return by $\BullShortfallPP{}$ percentage points in the bull regime, more than enough to explain the aggregate shortfall, but \DIFdelbegin \DIFdel{it beats }\DIFdelend \DIFaddbegin \DIFadd{on point estimates it leads }\DIFaddend buy-and-hold on every risk-adjusted metric considered: Sharpe ($\Shtest{}$ vs.\ $\BHsharpe{}$), Sortino ($\EnsSortino{}$ vs.\ $\BHSortino{}$), Calmar ($\Calmtest{}$ vs.\ $\Calmbh{}$), Ulcer Index ($\EnsUlcer{}$ vs.\ $\BHUlcer{}$), Martin ratio ($\EnsMartin{}$ vs.\ $\BHMartin{}$), and maximum drawdown ($\MDDtest{}$ vs.\ $\MDDbh{}$). Once the two series are matched on ex-ante volatility, the ensemble also \DIFdelbegin \DIFdel{beats }\DIFdelend \DIFaddbegin \DIFadd{leads }\DIFaddend buy-and-hold on total return, by $\VolScaledExcessPP{}$ percentage points, with materially smaller drawdown. \DIFaddbegin \DIFadd{As the significance analysis in Section~\ref{subsec:significance} makes clear, these advantages are point-estimate-level and do not attain conventional statistical significance on a single sub-two-year window, so we read them as consistent and economically meaningful rather than as statistical dominance. The FTSE 100 replication in Section~\ref{subsec:ftse} yields the same qualitative H1 pattern on }\FtseOOSDays{} \DIFadd{held-out trading days under UK-specific costs, supporting external validity even though the formal hypothesis is stated against WIG20. }\DIFaddend These observations instantiate the variance-reduction prediction of forecast-combination theory: the combination of heterogeneous directional forecasts lowers the variance of the equity curve, and the reduction is visible both in the volatility and in the drawdown profile.
...
Methodologically, the evaluation pipeline \DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend anchored walk-forward tuning of decision-agent parameters, calibrated frictions on every trade, ablation across single-agent removals, \DIFaddbegin \DIFadd{a comparison against an LSTM deep-learning baseline, }\DIFaddend regime decomposition, a wide cost-sensitivity sweep, and bootstrap significance tests \DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend closes the four gaps identified in Section~\ref{sec:introduction} and is reusable as a template for similar studies on smaller equity markets, where capital preservation in turbulent periods may matter more than chasing the next bull leg.
...
The directions outlined in Section~\ref{sec:limitations} \DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend cross-market generalization, intraday frequencies, fundamental and alternative data, learned and regime-conditional family weights, and a live paper-trading exercise \DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend are the natural extensions to leverage these contributions in the next iteration of the system.
