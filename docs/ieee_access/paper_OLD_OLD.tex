...
Each signal agent is described below. In all cases, the agent returns a neutral (hold) signal when the history is too short to compute the indicator, and the confidence value is zero outside the active signal bands.

\textbf{MACD (moving-average convergence-divergence).} Two exponential moving averages (EMAs) of the closing price are computed: a fast EMA (default span of $8$) and a slow EMA (default span of $17$). The MACD line is their difference and the signal line is an EMA of the MACD line (default span of $9$). The agent emits $+1$ when the MACD line crosses above the signal line and $-1$ when it crosses below. Confidence is computed from the current histogram value $h$, normalized by its standard deviation $\sigma$ over the last $50$ bars, as
\begin{equation}
\mathrm{confidence} = \tanh\!\left(\frac{|h|}{2\sigma}\right).
\end{equation}
Methodological family: momentum.
...
\end{cases}
\end{equation}
with confidence equal to the normalized distance of the RSI from the nearest threshold. Methodological family: mean-reversion.
...
\end{cases}
\end{equation}
where $\tau$ is a configurable threshold (default $0.02$). Confidence equals $|\mathrm{ROC}|$, clipped to $[0, 1]$. Methodological family: momentum.
...
\end{aligned}
\end{equation}
reflecting a prior that trend-following agents tend to be more informative than pure volatility agents on WIG20 daily data. The score is then compared against mode-dependent thresholds to produce the final decision:
\begin{equation}
\mathrm{action} =
\begin{cases}
...
Two families of baselines are used. The first is \emph{buy-and-hold WIG20}, an equity curve that invests the initial cash into the WIG20 index on the first date of the test window and holds the position until the end. Buy-and-hold is a standard benchmark in the evaluation of trading strategies \cite{dichtl2020}. The second family is a set of \emph{single-agent} runs in which the decision agent receives signals from only one signal agent; this produces five baselines, one per indicator. In addition, we run five \emph{leave-one-agent-out} configurations, each of which removes one agent from the ensemble. All baselines are evaluated on the same test window, use the same transaction costs, and adopt the same decision-agent configuration selected by the walk-forward protocol. The only exception is buy-and-hold, which has no decision-agent parameters.
