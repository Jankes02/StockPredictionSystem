...
\begin{abstract}
We study a multi-agent decision support system for stock trading on the Warsaw Stock Exchange (WSE), a Central and Eastern European market underrepresented in the algorithmic trading literature. The system aggregates signals from five heterogeneous technical-analysis agents through a configurable decision agent whose confidence-weighted score is grounded in forecast-combination theory, which predicts variance reduction when several imperfect forecasters are combined. The primary contribution is a rigorous and reusable evaluation framework, demonstrated through an in-depth empirical study of a deliberately transparent, theory-grounded ensemble that instantiates it. The framework addresses four common weaknesses of backtested trading studies: in-sample parameter selection, absent transaction costs, neglect of risk-adjusted performance, and lack of significance tests. An anchored walk-forward protocol tunes only the decision-agent parameters, a calibrated commission and slippage model is applied to every trade, risk-adjusted performance is summarized with the Sharpe, Sortino, Calmar, and Martin ratios and the Ulcer Index with a volatility-matched comparison and bootstrap confidence intervals, and the held-out window is decomposed into bull, correction, and sideways regimes. On the held-out test window for the twenty-stock blue-chip index WIG20, over \OOSDays{} trading days, the ensemble trails passive buy-and-hold on raw total return over the bull-dominated subperiod but improves every risk-adjusted metric considered and, once both series are scaled to the same ex-ante volatility, also raw total return; these differences are economically meaningful but, on a single sub-two-year window, do not reach statistical significance at conventional levels. A regime decomposition attributes the profile to smaller losses during corrections and higher returns during sideways phases, offset by lower bull-market returns. The same qualitative pattern is reproduced in an external validation on the FTSE 100 under UK-specific costs, and the ensemble remains competitive with an LSTM directional baseline evaluated under the identical protocol. Cost-sensitivity and ablation checks confirm robustness. The results support theory-grounded ensembles as risk-aware decision support rather than return enhancers.
\end{abstract}
